...
def handler(ctx, data: io.BytesIO = None):
    ...
    try:
        ...
        for task_row in tasks:
            try:
                content_raw = task_row.get('content')

                # Noneや空文字、辞書型などを考慮して安全に処理
                if isinstance(content_raw, dict):
                    task_content = content_raw
                elif isinstance(content_raw, str):
                    task_content = json.loads(content_raw or '{}')
                else:
                    task_content = {}
                ...
                logger.warning(f"Unexpected error parsing task {task_row.get('id')}: {str(ex)}")
                continue
